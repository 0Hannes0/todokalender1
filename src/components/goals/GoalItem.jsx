  ...
  return (
    <div className={`flex items-center gap-2.5 px-2 py-2 rounded-xl group hover:bg-surface-2 active:bg-surface-2 transition-colors ${goal.completed ? 'opacity-50' : ''}`}>
      ...
      <button onClick={() => onDelete(goal.id)} aria-label="Löschen"
        className="opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-opacity text-text-4 hover:text-red-400 focus:opacity-100 focus:outline-none w-7 h-7 flex items-center justify-center flex-shrink-0">
